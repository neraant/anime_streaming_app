...
import {
	addFavorite,
	removeFavorite
} from '../../../Services/firebaseFavoritesServices'

const ProfileCarouselItem = ({ anime, action }) => {
	const { user } = useUser()
	...
	return (
		...
		>
			...
			{action === 'favorite' && (
				<button
					className={`absolute z-1 top-5 right-5 cursor-pointer transition-all duration-300 ${
						isFavoriteAnime ? 'text-[#ad46ff]' : 'text-white'
					}`}
